...
class M3uParser:
    ...
    def parse_m3u(self, path: str, check_live: bool = True, enforce_schema: bool = True):
        ...
        """
        if aiohttp is None:
            self._check_live = False # Do not allow check_live is aiohttp is not available
        else:
    ...
    async def _parse_line(self, line_num: int):
        ...
        if line_info and stream_link:
            ...
            if (country != None or self._enforce_schema) and pycountry is not None:
            ...
            if (language != None or self._enforce_schema) and pycountry is not None:
